...
<body>
	...
	<?php
		$sqlLeker = "SELECT * from quizes where id = {$_SESSION["updateKerdesSorId"]}";
		$queryLeker = $connDB->prepare($sqlLeker);
		$queryLeker->execute();
		while ($row = $queryLeker->fetch(PDO::FETCH_ASSOC)){
		
			echo "<form action='kerdessor.php' method='post'>
					<input id='nev' name='nev' type='text' value='{$row["name"]}'>
	...
	<?php
		...
		function feltolt($connDB, $ids, $name){
				...
				try{
					$sqlUpdate = "UPDATE quizes SET name ='{$name}', questionIds='{$ids}', points='{$maxPoint}' WHERE id={$_SESSION["updateKerdesSorId"]}";
					$sqlBeir = "INSERT INTO quizes (name, questionIds, points)
								VALUES ('$name', '$ids', '$maxPoint')";
					$connDB->exec($sqlUpdate);
				} catch(PDOException $e){
